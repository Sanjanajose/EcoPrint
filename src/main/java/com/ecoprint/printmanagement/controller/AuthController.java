...
=======
    ...
    public ResponseEntity<?> registerUser(
            @Param(value = "The RegistrationRequest payload") @Valid @RequestPart("user") RegistrationRequest registrationRequest,
            @RequestPart(value = "profilePicture", required = false) MultipartFile profilePicture) {

        // Define allowed file types (JPEG and PNG)
        Set<String> allowedContentTypes = Set.of("image/jpeg", "image/png");

        // Validate file size (limit to 5MB)
        long maxFileSize = 5 * 1024 * 1024; // 5 MB in bytes
        if (profilePicture != null && profilePicture.getSize() > maxFileSize) {
        ...
        if (profilePicture != null && !allowedContentTypes.contains(profilePicture.getContentType())) {
        ...
        if (profilePicture != null && !profilePicture.isEmpty()) {
            try {
                profilePictureUrl = userService.uploadProfilePicture(profilePicture, 300, 300);  // Upload and resize
                registrationRequest.setProfilePicture(profilePictureUrl);  // Set the uploaded file URL
            } catch (IOException e) {
